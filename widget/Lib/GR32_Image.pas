...
type
  ...
  TWidgetPaintBox32 = class(TCustomControl)
  private
    FBuffer: TBitmap32;
  ...
  TPaintBox32 = class(TWidgetPaintBox32)
  private
...
{$ENDIF}
  ...
  TCustomImage32_ml = class(TWidgetPaintBox32)
  private
    FBitmap: TBitmap32;
  ...
  TImage32 = class(TCustomImage32_ml)
  published
    property Align;
    property Anchors;
    property AutoSize;
...
{$ENDIF}
  ...
  TCustomImgView32 = class(TCustomImage32_ml)
  private
    FCentered: Boolean;
...
{$IFNDEF FPC}
procedure TWidgetPaintBox32.CMInvalidate(var Message: TMessage);
begin
...
procedure TWidgetPaintBox32.AssignTo(Dest: TPersistent);
begin
  inherited AssignTo(Dest);
  if Dest is TWidgetPaintBox32 then
  begin
    FBuffer.Assign(TWidgetPaintBox32(Dest).FBuffer);
    TWidgetPaintBox32(Dest).FBufferOversize := FBufferOversize;
    TWidgetPaintBox32(Dest).FBufferValid := FBufferValid;
    TWidgetPaintBox32(Dest).FRepaintMode := FRepaintMode;
    TWidgetPaintBox32(Dest).FInvalidRects := FInvalidRects;
    TWidgetPaintBox32(Dest).FForceFullRepaint := FForceFullRepaint;
    TWidgetPaintBox32(Dest).FOptions := FOptions;
    TWidgetPaintBox32(Dest).FOnGDIOverlay := FOnGDIOverlay;
    TWidgetPaintBox32(Dest).FOnMouseEnter := FOnMouseEnter;
    TWidgetPaintBox32(Dest).FOnMouseLeave := FOnMouseLeave;
  end;
end;

procedure TWidgetPaintBox32.CMMouseEnter(var Message: {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  inherited;
  MouseEnter;
end;

procedure TWidgetPaintBox32.CMMouseLeave(var Message: {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
begin
  MouseLeave;
  inherited;
end;

constructor TWidgetPaintBox32.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TWidgetPaintBox32.Destroy;
begin
...
procedure TWidgetPaintBox32.DoPaintGDIOverlay;
begin
  if Assigned(FOnGDIOverlay) then FOnGDIOverlay(Self);
end;

procedure TWidgetPaintBox32.Flush;
begin
...
procedure TWidgetPaintBox32.Flush(const SrcRect: TRect);
var
  R: TRect;
begin
  if (FBuffer.Handle <> 0) then
...
function TWidgetPaintBox32.GetViewportRect: TRect;
begin
...
procedure TWidgetPaintBox32.Invalidate;
begin
  FBufferValid := False;
  inherited;
end;

procedure TWidgetPaintBox32.ForceFullInvalidate;
begin
...
procedure TWidgetPaintBox32.Loaded;
begin
  FBufferValid := False;
  inherited;
end;

procedure TWidgetPaintBox32.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TWidgetPaintBox32.MouseEnter;
begin
...
procedure TWidgetPaintBox32.MouseLeave;
begin
...
procedure TWidgetPaintBox32.Paint;
begin
...
procedure TWidgetPaintBox32.ResetInvalidRects;
begin
  FInvalidRects.Clear;
end;

procedure TWidgetPaintBox32.Resize;
begin
  ResizeBuffer;
  BufferValid := False;
  inherited;
...
procedure TWidgetPaintBox32.ResizeBuffer;
var
...
procedure TWidgetPaintBox32.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited;
...
procedure TWidgetPaintBox32.SetBufferOversize(Value: Integer);
begin
...
procedure TWidgetPaintBox32.WMEraseBkgnd(var Message: {$IFDEF FPC}TLmEraseBkgnd{$ELSE}TWmEraseBkgnd{$ENDIF});
begin
  Message.Result := 1;
end;

procedure TWidgetPaintBox32.WMGetDlgCode(var Msg: {$IFDEF FPC}TLMessage{$ELSE}TWmGetDlgCode{$ENDIF});
begin
  with Msg do
...
procedure TWidgetPaintBox32.WMPaint(var Message: {$IFDEF FPC}TLMPaint{$ELSE}TMessage{$ENDIF});
begin
...
procedure TWidgetPaintBox32.DirectAreaUpdateHandler(Sender: TObject;
...
procedure TWidgetPaintBox32.SetRepaintMode(const Value: TRepaintMode);
begin
  if Assigned(FRepaintOptimizer) then
  begin
    
    if (Value = rmOptimizer) and not(Self is TCustomImage32_ml) then
...
constructor TCustomImage32_ml.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TCustomImage32_ml.Destroy;
begin
  BeginUpdate;
  FPaintStages.Free;
...
procedure TCustomImage32_ml.BeginUpdate;
begin
  
  Inc(FUpdateCount);
end;

procedure TCustomImage32_ml.BitmapResized;
var
  W, H: Integer;
begin
  if AutoSize then
...
procedure TCustomImage32_ml.BitmapChanged(const Area: TRect);
begin
  Changed;
end;

function TCustomImage32_ml.BitmapToControl(const APoint: TPoint): TPoint;
begin
...
function TCustomImage32_ml.BitmapToControl(const APoint: TFloatPoint): TFloatPoint;
begin
...
procedure TCustomImage32_ml.BitmapResizeHandler(Sender: TObject);
begin
  BitmapResized;
end;

procedure TCustomImage32_ml.BitmapChangeHandler(Sender: TObject);
begin
  FRepaintOptimizer.Reset;
  BitmapChanged(Bitmap.Boundsrect);
end;

procedure TCustomImage32_ml.BitmapAreaChangeHandler(Sender: TObject;
  const Area: TRect; const Info: Cardinal);
var
  T, R: TRect;
...
procedure TCustomImage32_ml.BitmapDirectAreaChangeHandler(Sender: TObject;
  const Area: TRect; const Info: Cardinal);
var
  T, R: TRect;
  Width, Tx, Ty, I, J: Integer;
...
function TCustomImage32_ml.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
var
  W, H: Integer;
begin
  InvalidateCache;
...
procedure TCustomImage32_ml.Changed;
begin
  if FUpdateCount = 0 then
  begin
    Invalidate;
...
function TCustomImage32_ml.ControlToBitmap(const APoint: TPoint): TPoint;
begin
...
function TCustomImage32_ml.ControlToBitmap(const APoint: TFloatPoint): TFloatPoint;
begin
...
procedure TCustomImage32_ml.DoInitStages;
begin
  if Assigned(FOnInitStages) then FOnInitStages(Self);
end;

procedure TCustomImage32_ml.DoPaintBuffer;
var
...
procedure TCustomImage32_ml.DoPaintGDIOverlay;
var
  I: Integer;
begin
...
procedure TCustomImage32_ml.DoScaleChange;
begin
  if Assigned(FOnScaleChange) then FOnScaleChange(Self);
end;

procedure TCustomImage32_ml.EndUpdate;
begin
...
procedure TCustomImage32_ml.ExecBitmapFrame(Dest: TBitmap32; StageNum: Integer);
begin
  Dest.Canvas.DrawFocusRect(CachedBitmapRect);
end;

procedure TCustomImage32_ml.ExecClearBackgnd(Dest: TBitmap32; StageNum: Integer);
var
  C: TColor32;
  I: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Dest.FillRectS(0, 0, Self.Width, Top, C);          
        Dest.FillRectS(0, Bottom, Self.Width, Self.Height, C);  
        Dest.FillRectS(0, Top, Left, Bottom, C);      
        Dest.FillRectS(Right, Top, Self.Width, Bottom, C); 
      end;
    end;
  end;
end;

procedure TCustomImage32_ml.ExecClearBuffer(Dest: TBitmap32; StageNum: Integer);
begin
  Dest.Clear(Color32(Color));
end;

procedure TCustomImage32_ml.ExecControlFrame(Dest: TBitmap32; StageNum: Integer);
begin
  DrawFocusRect(Dest.Handle, Rect(0, 0, Width, Height));
end;

procedure TCustomImage32_ml.ExecCustom(Dest: TBitmap32; StageNum: Integer);
begin
  if Assigned(FOnPaintStage) then FOnPaintStage(Self, Dest, StageNum);
end;

procedure TCustomImage32_ml.ExecDrawBitmap(Dest: TBitmap32; StageNum: Integer);
var
  I, J, Tx, Ty: Integer;
  R: TRect;
begin
...
procedure TCustomImage32_ml.ExecDrawLayers(Dest: TBitmap32; StageNum: Integer);
var
  I: Integer;
  Mask: Cardinal;
begin
...
function TCustomImage32_ml.GetBitmapRect: TRect;
var
  Size: TSize;
begin
...
function TCustomImage32_ml.GetBitmapSize: TSize;
var
  Mode: TScaleMode;
...
function TCustomImage32_ml.GetOnPixelCombine: TPixelCombineEvent;
begin
  Result := FBitmap.OnPixelCombine;
end;

procedure TCustomImage32_ml.InitDefaultStages;
begin
...
procedure TCustomImage32_ml.Invalidate;
begin
  BufferValid := False;
  CacheValid := False;
  inherited;
...
procedure TCustomImage32_ml.InvalidateCache;
begin
...
function TCustomImage32_ml.InvalidRectsAvailable: Boolean;
begin
...
procedure TCustomImage32_ml.LayerCollectionChangeHandler(Sender: TObject);
begin
  Changed;
end;

procedure TCustomImage32_ml.LayerCollectionGDIUpdateHandler(Sender: TObject);
begin
  Paint;
end;

procedure TCustomImage32_ml.LayerCollectionGetViewportScaleHandler(Sender: TObject;
...
procedure TCustomImage32_ml.LayerCollectionGetViewportShiftHandler(Sender: TObject;
...
procedure TCustomImage32_ml.Loaded;
begin
  inherited;
  DoInitStages;
end;

procedure TCustomImage32_ml.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  Layer: TCustomLayer;
begin
  inherited;
...
procedure TCustomImage32_ml.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  Layer: TCustomLayer;
begin
  inherited;
...
procedure TCustomImage32_ml.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  Layer: TCustomLayer;
begin
  if Layers.MouseEvents then
...
procedure TCustomImage32_ml.MouseDown(Button: TMouseButton;
...
procedure TCustomImage32_ml.MouseUp(Button: TMouseButton; Shift: TShiftState;
...
procedure TCustomImage32_ml.MouseLeave;
begin
...
procedure TCustomImage32_ml.PaintTo(Dest: TBitmap32; DestRect: TRect);
var
...
procedure TCustomImage32_ml.Resize;
begin
  InvalidateCache;
  inherited;
end;

procedure TCustomImage32_ml.SetBitmap(Value: TBitmap32);
begin
  InvalidateCache;
  FBitmap.Assign(Value);
end;

procedure TCustomImage32_ml.SetBitmapAlign(Value: TBitmapAlign);
begin
  InvalidateCache;
  FBitmapAlign := Value;
  Changed;
...
procedure TCustomImage32_ml.SetLayers(Value: TLayerCollection);
begin
  FLayers.Assign(Value);
end;

procedure TCustomImage32_ml.SetOffsetHorz(Value: TFloat);
begin
...
procedure TCustomImage32_ml.SetOffsetVert(Value: TFloat);
begin
...
procedure TCustomImage32_ml.SetOnPixelCombine(Value: TPixelCombineEvent);
begin
  FBitmap.OnPixelCombine := Value;
  Changed;
end;

procedure TCustomImage32_ml.SetScale(Value: TFloat);
begin
...
procedure TCustomImage32_ml.SetScaleX(Value: TFloat);
begin
  if Value < 0.001 then Value := 0.001;
  if Value <> FScaleX then
  begin
...
procedure TCustomImage32_ml.SetScaleY(Value: TFloat);
begin
  if Value < 0.001 then Value := 0.001;
...
procedure TCustomImage32_ml.SetScaleMode(Value: TScaleMode);
begin
...
procedure TCustomImage32_ml.SetupBitmap(DoClear: Boolean = False; ClearColor: TColor32 = $FF000000);
begin
  FBitmap.BeginUpdate;
  with GetViewPortRect do
...
procedure TCustomImage32_ml.SetXForm(ShiftX, ShiftY, ScaleX, ScaleY: TFloat);
begin
  CachedShiftX := ShiftX;
  CachedShiftY := ShiftY;
  CachedScaleX := ScaleX;
...
procedure TCustomImage32_ml.SetRepaintMode(const Value: TRepaintMode);
begin
  inherited;
...
procedure TCustomImage32_ml.Update(const Rect: TRect);
begin
  if FRepaintOptimizer.Enabled then
    FRepaintOptimizer.AreaUpdateHandler(Self, Rect, AREAINFO_RECT);
end;

procedure TCustomImage32_ml.UpdateCache;
begin
...
begin
  ...
  begin
    if W > Sz.Cx + 2 * ScaledOversize then 
      OffsetHorz := (W - Sz.Cx) * 0.5
    else
      OffsetHorz := -HScroll.Position + ScaledOversize;

    if H > Sz.Cy + 2 * ScaledOversize then 
