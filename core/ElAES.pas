...
function AESEncryptBuffer128(dest: PAnsiChar; destLen: PLongint;
   source: PAnsiChar; Count: Longint; const strKey: AnsiString): integer;
function AESDecryptBuffer128(dest: PAnsiChar; destLen: PLongint;
   source: PAnsiChar; Count: Longint; const strKey: AnsiString): integer;
...
var
  Key: TAESKey128;
  ExpandedKey: TAESExpandedKey128;
  TempIn, TempOut: TAESBuffer;//TAESBuffer = array [0..15] of byte;
begin
  Result := 0;
  if Count = 0 then exit;
   // Prepare key...
  FillChar(Key, SizeOf(Key), 0);
  Move(PAnsiChar(strKey)^, Key, Min(SizeOf(Key), Length(strKey)));
  ExpandAESKeyForEncryption(Key, ExpandedKey);
  ...
  while Count >= SizeOf(TAESBuffer) do
  begin
    Move(source^,TempIn, SizeOf(TempIn));
    EncryptAES(TempIn, ExpandedKey, TempOut);
    Move(TempOut,PAnsiChar(dest)^, SizeOf(TempOut));
    Dec(Count, SizeOf(TAESBuffer));
    Inc(source,SizeOf(TAESBuffer));
    Inc(dest,SizeOf(TAESBuffer));
    Inc(destLen^,SizeOf(TAESBuffer));
  end;
  if Count > 0 then
  begin
    Move(source^,TempIn, count);
    FillChar(TempIn[Count], SizeOf(TempIn) - Count, 0);
    EncryptAES(TempIn, ExpandedKey, TempOut);
    Move(TempOut,PAnsiChar(dest)^, SizeOf(TempOut));
    Inc(destLen^,SizeOf(TAESBuffer));
...
begin
  Result := 0;
  if Count = 0 then exit;
  if (Count mod SizeOf(TAESBuffer)) > 0 then  Exit;
   // Prepare key...
  FillChar(Key, SizeOf(Key), 0);
  Move(PAnsiChar(strKey)^, Key, Min(SizeOf(Key), Length(strKey)));
  ExpandAESKeyForEncryption(Key, ExpandedKey);
  ...
  while Count >= SizeOf(TAESBuffer) do
  begin
    CopyMemory(@TempIn,source+destLen^, SizeOf(TempIn));
    DecryptAES(TempIn, ExpandedKey, TempOut);
    CopyMemory(dest+destLen^,@TempOut, SizeOf(TempOut));
    Dec(Count, SizeOf(TAESBuffer));
    Inc(destLen^,SizeOf(TAESBuffer));
  end;
